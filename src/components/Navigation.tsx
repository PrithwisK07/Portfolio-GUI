...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ScrollTrigger for Swapping Inline Nav to Pill Nav
      ScrollTrigger.create({
        start: 1500, 
        onEnter: () => {
          // Hide inline nav, show pill
  ...
  return (
    <>
      ...
      <nav className="global-nav-reveal opacity-0 fixed top-0 left-0 w-full p-6 md:p-10 flex justify-end items-start z-[10001] pointer-events-none" id="main-nav">
        ...
        {/* INLINE NAV (Visible on Load) */}
        <div className="inline-nav absolute top-6 md:top-10 right-6 md:right-10 flex items-center gap-8 text-black mix-blend-difference pointer-events-auto font-medium text-sm md:text-base tracking-wide z-10 transition-opacity">
           {menuLinks.map((link) => (
             <a 
               key={`inline-${link.label}`}
               href={link.href}
               onClick={(e) => handleScrollTo(e, link.href)}
               className="hover:opacity-60 transition-opacity cursor-pointer-custom"
             >
               {link.label}
             </a>
           ))}
        ...
        {/* PILL NAV (Hidden on Load, slides down on scroll) */}
